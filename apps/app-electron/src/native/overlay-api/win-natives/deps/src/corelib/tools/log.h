...
inline constexpr size_t file_name_offset(const T(&str)[S], size_t i = S - 1)
{
    return (str[i] == '/' || str[i] == '\\') ? i + 1 : (i > 0 ? file_name_offset(str, i - 1) : 0);
}

template <typename T>
inline constexpr size_t file_name_offset(T(&str)[1])
{
    return 0;
}

#define __FILENAME__ &__FILE__[file_name_offset(__FILE__)]
