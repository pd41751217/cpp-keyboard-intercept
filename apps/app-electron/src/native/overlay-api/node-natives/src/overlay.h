...
public:
    ...
    Napi::Value translateInputEvent(const Napi::CallbackInfo &info)
    {
        ...
        if ((msg >= WM_KEYFIRST && msg <= WM_KEYLAST) || (msg >= WM_SYSKEYDOWN && msg <= WM_SYSDEADCHAR))
        {
        ...
        else if (msg >= WM_MOUSEFIRST && msg <= WM_MOUSELAST)
        {
            ...
            if (msg == WM_LBUTTONDOWN || msg == WM_RBUTTONDOWN || msg == WM_MBUTTONDOWN || msg == WM_XBUTTONDOWN || msg == WM_LBUTTONDBLCLK || msg == WM_RBUTTONDBLCLK || msg == WM_MBUTTONDBLCLK || msg == WM_XBUTTONDBLCLK)
            {
    ...
    void notifyGameLog(std::uint32_t pid, const std::string &logMessage)
    {
        if (eventCallback_)
        {
            Napi::HandleScope scope(eventCallback_->env);
            Napi::Object object = Napi::Object::New(eventCallback_->env);
            object.Set("pid", Napi::Value::From(eventCallback_->env, pid));
            object.Set("message", Napi::Value::From(eventCallback_->env, logMessage));
            eventCallback_->callback.MakeCallback(eventCallback_->receiver.Value(), {Napi::Value::From(eventCallback_->env, "game.log"), Napi::Value::From(eventCallback_->env, object)});
        }
    }
...
#define OVERLAY_DISPATCH(type, Msg)                                                  \
    ...
    break;
            ...
            switch (stdxx::hash(ipcMsg.type.c_str()))
            {
                ...
                OVERLAY_DISPATCH("game.log", GameLog);
    ...
    void _onGameLog(std::uint32_t pid, const std::shared_ptr<overlay::GameLog> &overlayMsg)
    {
        node_async_call::async_call([this, pid, overlayMsg]()
                                    { notifyGameLog(pid, overlayMsg->message); });
    }
};
