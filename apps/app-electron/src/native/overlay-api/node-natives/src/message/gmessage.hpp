...
namespace overlay
{
    ...
    JSON_AUTO(WindowFrameBuffer, type, windowId, textureHandle, width, height)

    struct WindowSharedTextureHandle : public GMessage
    {
        GMESSAGE_AUTO("window.sharedtexturehandle");

        std::uint32_t windowId = 0;
        std::uint32_t sharedTextureHandle;
        int width = 0;
        int height = 0;
    };

    JSON_AUTO(WindowSharedTextureHandle, type, windowId, sharedTextureHandle, width, height)
...
JSON_AUTO(MovingSpeedCommand, type, speed)
    ...
    struct GameLog : public GMessage
    {
        GMESSAGE_AUTO("game.log");

        std::string message;
    };

    JSON_AUTO(GameLog, type, message)
